...
//자주 하는 질문 목록
    @Query(value = "SELECT quest_title, quest_content_fp, quest_category_id from quest_commen", countQuery = "SELECT COUNT(*) from quest_commen", nativeQuery = true)
    Page<QuestCommenDto> questCommenListAll(Pageable pageable);

    @Query(value = "SELECT quest_title, quest_content_fp, quest_category_id from quest_commen where quest_category_id = :quest_category_id", countQuery = "SELECT COUNT(*) from quest_commen where quest_category_id = :quest_category_id", nativeQuery = true)
...
//내 질문 목록
    ...
    @Query(value = "select quest_title, quest_content_fp, quest_answer_fp from quest_private where questioner_id = :questioner_id", countQuery = "SELECT COUNT(*) from quest_private where questioner_id = :questioner_id", nativeQuery = true)
